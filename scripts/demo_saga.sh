...
saga_status() {
  ...
  curl -sf "$CLAIM_URL/claims/$claim_id/saga" | python3 -m json.tool
}
...
CLAIM_H=$(curl -sf -X POST "$CLAIM_URL/claims" \
  -H "Authorization: Bearer $TOKEN_HAPPY" \
  -H "Content-Type: application/json" \
...
curl -sf -X POST "$CLAIM_URL/claims/$CLAIM_H_ID/approve" \
...
echo -e "\n▶ Step 3: SubmitClaim → ReserveItem..."
CLAIM_C=$(curl -sf -X POST "$CLAIM_URL/claims" \
  -H "Authorization: Bearer $TOKEN_COMP" \
  -H "Content-Type: application/json" \
...
curl -sf -X POST "$CLAIM_URL/claims/$CLAIM_C_ID/reject" \
