...
curl -sf "$CLAIM_URL/health" | python3 -m json.tool
curl -sf "$NOTIF_URL/health" | python3 -m json.tool

echo -e "\n[2] Register users (Item Service — application auth, not a separate microservice)..."
...
echo -e "\n[6] Submit claim (Alice) → Claim/Recovery Service..."
...
curl -sf -X POST "$CLAIM_URL/claims/$CLAIM_ID/approve" \
