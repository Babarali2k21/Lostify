...
echo ""
echo "Health checks:"
curl -sf "http://localhost:8001/health" && echo "  user-service: ok" || echo "  user-service: FAILED"
curl -sf "http://localhost:8002/health" && echo "  item-service: ok" || echo "  item-service: FAILED"
...
echo "  User Service:         http://${PUBLIC_IP}:8001/health"
echo "  Item Service:         http://${PUBLIC_IP}:8002/health"
